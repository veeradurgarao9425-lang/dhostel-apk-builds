  ...
  return (
    <View style={s.root}>
      ...
      <ScrollView
        ...
        contentContainerStyle={s.filterRow} style={s.filterBar}
      >
        {FILTER_CATS.map(cat => {
          const meta = CATS[cat];
          ...
          return (
            <TouchableOpacity
              key={cat}
              style={[s.filterPill, active && { backgroundColor: BLUE, borderColor: BLUE }]}
              onPress={() => setActiveFilter(cat)} activeOpacity={0.7}
            >
              {meta && (
                <View style={[s.filterPillDot, { backgroundColor: active ? 'rgba(255,255,255,0.3)' : meta.bg }]}>
                  <meta.Icon size={12} color={active ? WHITE : meta.color} strokeWidth={2} />
                </View>
              )}
              ...
              <View key={date} style={s.group}>
                ...
                <View style={s.groupHeader}>
                  <View>
                    ...
                    <Text style={s.groupCount}>{items.length} transaction{items.length > 1 ? 's' : ''}</Text>
                  </View>
                  {/* Big attractive total */}
                ...
                <View style={s.groupCard}>
                  {items.map((item, idx) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={item.id}
                        style={[s.row, idx < items.length - 1 && s.rowDivider]}
                        activeOpacity={0.7}
                      >
                        ...
                        <View style={{ alignItems: 'flex-end' }}>
                          ...
                          <View style={[s.catPill, { backgroundColor: meta.bg }]}>
                            <Text style={[s.catPillText, { color: meta.color }]}>{item.cat}</Text>
                          </View>
                        </View>
                      </TouchableOpacity>
                    );
...
const s = StyleSheet.create({
  ...
  searchBox: {
    ...
    borderRadius: 12, marginHorizontal: 16, marginBottom: 14,
  ...
  // Date group header — attractive with big amount badge
  groupHeader: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    marginBottom: 10,
  },
  ...
  groupTotalBadge: {
    borderRadius: 14, paddingHorizontal: 14, paddingVertical: 8, alignItems: 'flex-end',
