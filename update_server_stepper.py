...
# First, find the start of the `const html = ` assignment
start_html_idx = content.find('const html = `')
if start_html_idx == -1:
    print("Could not find const html = `")
    sys.exit(1)

# Find the end of the `const html = ` block (where res.status(200).send(html); starts)
end_html_idx = content.find('res.status(200).send(html);')
if end_html_idx == -1:
    print("Could not find res.status(200).send(html);")
    sys.exit(1)

# Replace the HTML block
new_content = content[:start_html_idx] + get_html + "\n  " + content[end_html_idx:]

with open(r'c:\dhostel-main\backend\src\server.ts', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("HTML block replaced successfully with stepper UI.")
