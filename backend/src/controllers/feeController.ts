...
export const getStudentPaymentHistory = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Owner (role 2): always scoped to their own hostel. Admin/Super Admin
    // (role 1): this endpoint is already scoped by studentId, so no further
    // hostel restriction applies (global access across all hostels).
    if (user?.role_id === 2) {
      const scopedHostelId = resolveScopedHostelId(user, undefined);
      if (!scopedHostelId) {
        return res.status(403).json({
          success: false,
          error: 'Your account is not linked to any hostel.'
  ...
  try {
    ...
    try {
      ...
      sendNotificationToStudent(
        ...
        'Payment',
        'Payment Received ✅',
        `Payment of ₹${amount_paid} has been recorded for your rent (Receipt: ${receiptNumber}).`,
        'High',
...
export const getAvailableMonths = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Owner (role 2): always scoped to their own hostel. Admin/Super Admin
    // (role 1): global across all hostels (no filter).
    if (user?.role_id === 2) {
      const scopedHostelId = resolveScopedHostelId(user, undefined);
      if (!scopedHostelId) {
        return res.status(403).json({
