...
export async function patchDatabaseSchema() {
  ...
  try {
    ...
    try {
      ...
      // One row per hostel: what the developer has agreed to charge that hostel,
      // how often, and when the next instalment falls due. `hostel_id` is UNIQUE
      // so the upsert in developerFinanceController can rely on ON DUPLICATE KEY.
      await db.raw(`
      ...
      // Every instalment the developer has actually received. Collected revenue is
      // SUM(amount) here — never derived from hostel_billing.agreed_amount, so
      // "expected" and "received" can never be conflated.
      await db.raw(`
      ...
      // The developer's own infrastructure costs — server, storage, email, domain.
      // Deliberately separate from the per-hostel `expenses` table, which belongs
      // to hostel owners and must not be mixed into platform net income.
      await db.raw(`
      ...
      // owning hostel and must never surface in an owner's feed.
      await db.raw(`
      ...
      try {
        ...
        const indexList = [
          ...
          { table: 'monthly_fees', name: 'idx_monthly_fees_hostel_status_id', cols: '(hostel_id, fee_status_id)' },
          { table: 'monthly_fees', name: 'idx_monthly_fees_hostel_due_date', cols: '(hostel_id, due_date)' },
          ...
          { table: 'income', name: 'idx_income_hostel_date', cols: '(hostel_id, income_date)' },
          { table: 'rooms', name: 'idx_rooms_hostel_number', cols: '(hostel_id, room_number)' },
          { table: 'rooms', name: 'idx_rooms_hostel_floor', cols: '(hostel_id, floor_number)' },
          { table: 'students', name: 'idx_students_hostel_status_room', cols: '(hostel_id, status, room_id)' },
          { table: 'students', name: 'idx_students_vacate', cols: '(hostel_id, status, vacate_notice_date)' },
          { table: 'students', name: 'idx_students_hostel_first_name', cols: '(hostel_id, status, first_name)' },
          ...
          { table: 'complaints', name: 'idx_complaints_hostel_status_id', cols: '(hostel_id, status_id)' },
